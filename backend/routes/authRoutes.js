...
// ── POST /api/auth/login ───────────────────────────────────────────────────
// FIX 1: Admin couldn't login — status check rejected users with null status.
//         Now only blocks if status is explicitly 'pending' or 'rejected'.
// FIX 2: JWT payload used { userId } but all controllers use req.user.id.
//         Now uses { id } throughout.
// FIX 3: Login response was missing user object that AuthContext.login() expects.
//         Now returns { token, role, user: { id, email, name, role } }.
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    // Only block explicitly pending/rejected — null status (legacy rows) is treated as approved
