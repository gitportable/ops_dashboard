...
router.get("/my-tasks",   auth, issueController.getMyTasks || ((req,res) => res.status(501).json({message: "getMyTasks not implemented yet"})));
router.get("/my-issues",  auth, issueController.getMyIssues || ((req,res) => res.status(501).json({message: "getMyIssues not implemented yet"})));
router.get("/project/:projectId", auth, issueController.getIssuesByProject || ((req,res) => res.status(501).json({message: "getIssuesByProject not implemented yet"})));
router.get("/batch/:batchLot", auth, issueController.getIssuesByBatch);

router.get("/", auth, issueController.getAllIssues);

router.post("/", auth, rbac("developer", "tester", "admin", "superadmin"), issueController.createIssue);

router.put("/:id/status", auth, rbac("developer", "tester", "admin", "superadmin"), issueController.updateIssueStatus || issueController.updateIssue);

router.put("/:id", auth, rbac("developer", "tester", "admin", "superadmin"), issueController.updateIssue);

router.delete("/:id", auth, rbac("admin", "superadmin"), issueController.deleteIssue);
...
// // ── IMPORTANT: Specific paths MUST come before parameterized /:id routes ──
// router.get('/my-tasks', auth, issueController.getMyTasks);   
// // GET all (role-filtered internally)
// router.get("/", controller.getAllIssues);

// // GET my tasks (developer — issues from assigned projects)
// router.get("/my-tasks", controller.getMyTasks);

// // GET my issues (tester — issues from assigned projects)
...
// // PUT update status only — must come BEFORE /:id to avoid Express matching /:id first
