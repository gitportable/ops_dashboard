// src/components/IssueTableDeveloper.js
import React, { useContext, useState } from "react";
import { AuthContext } from "../auth/AuthContext";
import { updateIssueStatus, updateIssue } from "../api/issueApi";
...
const StatusBadge = ({ status }) => {
  const map = {
    ...
    inspection:  { bg: "#fef3c7", color: "#d97706", label: "Inspecting" },
...
const SeverityBadge = ({ severity }) => {
  ...
  const s = colors[(severity || "medium").toLowerCase()] || colors.medium;
  return (
    <span style={{
      padding: "3px 10px", borderRadius: 99, fontSize: "0.72rem", fontWeight: 700,
...
const IssueTableDeveloper = ({ issues = [], onRefresh }) => {
  ...
  const handleSubtaskToggle = async (issue, subtaskIndex) => {
    const updatedSubtasks = [...(issue.subtasks || [])];
    if (typeof updatedSubtasks[subtaskIndex] === 'string') {
        // Migration if subtasks was array of strings
        updatedSubtasks[subtaskIndex] = { text: updatedSubtasks[subtaskIndex], done: true };
    } else {
    ...
    try {
      await updateIssue(issue.issueid, { subtasks: updatedSubtasks });
      onRefresh?.();
    } catch {
  ...
  const getNextActions = (status) => {
    ...
    if (s === "open") return [
      { label: "Start", next: "In Progress", color: "#2563eb" },
      { label: "Inspect", next: "Inspection", color: "#f59e0b" }
    ];
    if (s === "inspection") return [
      { label: "Approve", next: "Approved", color: "#16a34a" },
      { label: "Fix Needed", next: "In Progress", color: "#2563eb" }
    ];
    if (s === "approved") return [{ label: "Resolve", next: "Done", color: "#16a34a" }];
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 1200 }}>
          <thead>
            <tr style={{ background: "#f8fafc", borderBottom: "2px solid #e5e7eb" }}>
              {["#", "Type", "Priority", "Team", "Assignee", "Stage", "Status", "Severity", "Progress", "PR / Notes", "Actions"].map((h) => (
                <th key={h} style={{
          ...
          <tbody>
            {issues.map((issue, index) => {
              const id = issue.issueid || issue.issue_id || issue.IssueID || `dev-temp-${index}`;
                ...
                <React.Fragment key={id}>
                  ...
                  >
                    ...
                    <td style={{ padding: "12px 14px", fontSize: "0.85rem", color: "#374151" }}>
                      <span style={{ fontWeight: 600, color: issue.priority === 'High' || issue.priority === 'Critical' ? '#ef4444' : '#64748b' }}>
                    ...
                    <td style={{ padding: "12px 14px" }}>
                       {issue.subtasks?.length > 0 && (
                         <div style={{ fontSize: '0.7rem', color: '#64748b' }}>
                           {issue.subtasks.filter(s => s.done).length}/{issue.subtasks.length} tasks
                         </div>
                       )}
                    </td>
                    <td style={{ padding: "12px 14px" }} onClick={(e) => e.stopPropagation()}>
                      <input
                        placeholder="PR / Notes"
                        value={prLinks[id] || ""}
                        onChange={(e) => setPrLinks((prev) => ({ ...prev, [id]: e.target.value }))}
                        style={{
                          padding: "4px 8px", fontSize: "0.78rem", borderRadius: 6,
                          border: "1px solid #e5e7eb", outline: "none", width: 120,
                        }}
                      />
                    </td>
                    <td style={{ padding: "12px 14px" }} onClick={(e) => e.stopPropagation()}>
                  ...
                  {expandedId === id && (
                    <tr style={{ background: "#f0f7ff" }}>
                      <td colSpan={12} style={{ padding: "12px 20px", fontSize: "0.83rem", color: "#374151" }}>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2rem" }}>
                          <div style={{ borderRight: "1px solid #e2e8f0", paddingRight: "1rem" }}>
                            <h4 style={{ marginBottom: 10, fontSize: '0.85rem', fontWeight: 700 }}>Details & Context</h4>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
                              <div><strong>Description:</strong><br />{issue.description || "No description provided."}</div>
                              <div><strong>Created:</strong> {issue.createddate ? new Date(issue.createddate).toLocaleDateString() : "Unknown"}</div>
                              {issue.batch_lot && <div><strong>Batch/Lot:</strong> {issue.batch_lot}</div>}
                              {issue.rca && <div style={{ gridColumn: 'span 2' }}><strong>RCA:</strong> {issue.rca}</div>}
                              {issue.capa && <div style={{ gridColumn: 'span 2' }}><strong>CAPA:</strong> {issue.capa}</div>}
                            </div>
                          </div>
                          <div>
                            <h4 style={{ marginBottom: 10, fontSize: '0.85rem', fontWeight: 700 }}>Subtasks & Checklist</h4>
                            {issue.subtasks?.length > 0 ? (
                                    ...
                                    {issue.subtasks.map((st, i) => (
                                        <label key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                                            <input 
                                                ...
                                                onChange={() => handleSubtaskToggle(issue, i)}
                                            />
                                ...
                                <div style={{ color: '#94a3b8', fontStyle: 'italic' }}>No subtasks created for this issue.</div>
                            )}
                          </div>
                        </div>
                      </td>
...
//     try {
//       await updateIssueStatus(id, newStatus);
//       showToast(`Issue #${id} → "${newStatus}"`, 'success');
//       onRefresh?.();
//     } catch {
//       showToast(`Could not update issue #${id}. Check connection.`, 'error');
//     } finally {
...
//   /* ── Next valid status transitions for developer ── */
//   const getNextActions = (status) => {
//     const s = (status || '').toLowerCase();
//     if (s === 'open')        return [{ label: '⚡ Start', next: 'In Progress', color: '#2563eb' }];
//     if (s === 'in progress') return [
...
//       <div style={{ marginBottom: 8, fontSize: '0.78rem', color: '#9ca3af', fontStyle: 'italic' }}>
//         Developer view — follow the status workflow: Open → In Progress → In Review → Done.
//       </div>
...
//                         value={prLinks[id] || ''}
//                         onChange={e => setPrLinks(prev => ({ ...prev, [id]: e.target.value }))}
//                         style={{ padding: '4px 8px', fontSize: '0.78rem', borderRadius: 6, border: '1px solid #e5e7eb', outline: 'none', width: 150 }}
//                       />
//                     </td>
...
//                         value={effortMap[id] || ''}
//                         onChange={e => setEffortMap(prev => ({ ...prev, [id]: e.target.value }))}
//                         style={{ padding: '4px 8px', fontSize: '0.78rem', borderRadius: 6, border: '1px solid #e5e7eb', outline: 'none', width: 70 }}
//                       />
//                     </td>
...
//                             onClick={() => handleStatusUpdate(issue, a.next)}
//                             disabled={loadingId === `${id}-${a.next}`}
//                             style={{ padding: '4px 10px', fontSize: '0.75rem', fontWeight: 700, borderRadius: 6, border: 'none', cursor: 'pointer',
//                               background: `${a.color}15`, color: a.color, border: `1px solid ${a.color}33` }}>
//                             {loadingId === `${id}-${a.next}` ? '…' : a.label}
//                           </button>
//                         ))}
//                         {isDone && <span style={{ fontSize: '0.78rem', color: '#16a34a', fontWeight: 600 }}>✓ Complete</span>}
//                       </div>
//                     </td>
//                   </tr>

//                   {expandedId === id && (
//                     <tr key={`${id}-exp`} style={{ background: '#f0f7ff' }}>
//                       <td colSpan={8} style={{ padding: '12px 20px', fontSize: '0.83rem', color: '#374151' }}>
//                         <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem' }}>
//                           <div><strong>Description:</strong><br />{issue.description || 'No description provided.'}</div>
//                           <div><strong>Created:</strong> {issue.createddate || 'Unknown'}</div>
//                           <div><strong>Closed:</strong> {issue.closeddate || 'Not yet'}</div>
//                           {prLinks[id] && (
//                             <div><strong>PR:</strong>{' '}
//                               <a href={prLinks[id].startsWith('http') ? prLinks[id] : `https://${prLinks[id]}`}
//                                 target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb', fontSize: '0.83rem' }}>
//                                 {prLinks[id]}
//                               </a>
//                             </div>
//                           )}
